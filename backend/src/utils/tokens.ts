...
export default {
  // generate tokens
  generate: {
    accessToken: (payload: TJwtPayload, options: object = {}): string => {
    ...
    refreshToken: (payload: TJwtPayload, options: object = {}): string => {
  ...
  decompose: {
    accessToken: (token: string): TDecomposeResult => {
      try {
        ...
        const payload = jwt.verify(token, config.authSecret) as TJwtPayload;
        return { payload };
      } catch (err) {
    ...
    refreshToken: (token: string): TDecomposeResult => {
      try {
        ...
        const payload = jwt.verify(token, config.refreshSecret) as TJwtPayload;
        return { payload };
      } catch (err) {
        if (err instanceof jwt.TokenExpiredError) {
          return { payload: null };
