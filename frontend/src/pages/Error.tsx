...
const ErrorPage: React.FC<ErrorPagePropsType> = ({ title, body, details }) => {
  ...
  const bodyText = `${body ?? "We encountered an error"}. Report this by emailing us the details below.`;
  return (
      ...
      <div className="flex flex-col justify-start gap-2 px-5 w-full md:max-w-[500px] m-auto p-10 md:pt-30">
        <div>
          <h2 className="text-2xl md:text-3xl mb-5">
            <span className="italic font-semibold">Opps!</span>&nbsp;
            {title ?? "Something went wrong"}
          </h2>
          <p className="text-neutral">{bodyText}</p>
        </div>
        <div>
          <Accordion type="single" collapsible className="w-full ">
            <AccordionItem value="error-details">
              ...
              <AccordionContent className="justify-self-start max-h-[240px] md:max-h-[300px] overflow-y-scroll border-neutral-200 rounded-bl-md rounded-br-md border-[0.1px] overflow-y-scroll w-full pb-0">
                ...
                <div className="text-xs flex flex-col px-5 wrap-anywhere text-wrap break-normal whitespace-wrap max-w-[100%]">
                  {details
                    ? details
